...
test('remote Subsonic metadata is never interpolated into HTML templates', () => {
  for (const template of dynamicHtmlTemplates()) {
    assert.doesNotMatch(
      template,
      /\$\{\s*(?:server\.(?:name|url)|item\.(?:id|name)|subtitle|e(?:\.message)?)\s*\}/,
    )
  }
})

test('Subsonic item IDs are not embedded in inline event handlers', () => {
  assert.doesNotMatch(source, /<[^>]*\sonclick\s*=/i)
  assert.match(source, /addEventListener\(['"]click['"][\s\S]*?_importSingle\(item\.id\)/)
})

test('Subsonic remote labels use text-only DOM assignments', () => {
  ...
  assert.match(source, /textContent\s*=\s*subtitle/)
})
