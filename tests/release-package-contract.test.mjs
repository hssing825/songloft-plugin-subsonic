...
const bundlePath = process.env.SUBSONIC_PLUGIN_ZIP
  || join(repoRoot, 'dist', 'subsonic.jsplugin.zip')
...
test('release ZIP has one unambiguous entry and valid declared hashes', async () => {
  ...
  const sourceManifest = JSON.parse(readFileSync(join(repoRoot, 'plugin.json'), 'utf8'))
  const declaredEntry = zip.file(manifest.main)

  assert.ok(declaredEntry, `manifest main is missing: ${manifest.main}`)
  assert.equal(sourceManifest.main, manifest.main)
  assert.equal(sourceManifest.entryHash, manifest.entryHash)
  assert.equal(sourceManifest.zipHash, manifest.zipHash)
  assert.equal(
  ...
  const names = files.filter(name => name !== 'plugin.json').sort()
